...
> **Fase 4 CONCLUÍDA (2026-07-05):** funil comercial Kanban (`/funil`) — leads do 1º contato ao contrato fechado, alimentados pelo ai-attendant/WhatsApp + Cal.com. Lotes 1–5: base de dados (migration 040 + pré-cadastro de cliente), rotas de integração fail-closed (`x-simas-token`, webhook Cal.com HMAC idempotente por `uid`, cron diário), kanban (dnd-kit, filtros, badges), drawer (cliente/documentos/timeline + gerar contrato/procuração), promoção do cliente no contrato_fechado, métricas (`/funil/metricas`). Também corrigiu bug de middleware que redirecionava rotas de API autônomas (`/api/funil`, `/api/cron`, `/api/webhooks`) para `/login` — o webhook D4Sign e o cron de lembretes estavam inertes em produção. Integração validada ponta a ponta em produção. Plano: [PLANO-FASE-4-OPUS.md](PLANO-FASE-4-OPUS.md) · Docs: [funil-comercial.md](funil-comercial.md), [INTEGRACAO-AI-ATTENDANT.md](INTEGRACAO-AI-ATTENDANT.md). **Paradas do dono:** aplicar snippets no VPS (ai-attendant) + configurar webhooks nas 2 contas Cal.com + validar visual.
> **Fase 3 CONCLUÍDA (2026-07-04):** mineração de teses das peças do escritório — upload → IA extrai teses (só citações literais, anonimizadas) → verificador B5.2 checa cada citação → advogado revisa/aprova na Biblioteca → tese aprovada fundamenta a geração. Base de teses migrada para o banco (por tenant). Plano: [PLANO-FASE-3-OPUS.md](PLANO-FASE-3-OPUS.md). **Parada:** subir 3–5 peças reais e aprovar as primeiras teses.
