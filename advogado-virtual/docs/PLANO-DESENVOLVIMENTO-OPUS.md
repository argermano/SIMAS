...
- Commit `9aad5c9` — **Lote 2 (SSE robusto) CONCLUÍDO**: `src/lib/sse-parser.ts` (parser com buffer de linha parcial, 9 testes); `StreamingText.tsx` usa o parser no componente e no hook; `startStream` retorna `stopReason`; `TelaAtendimento`/`TelaRefinamento` avisam em `max_tokens`; `ResultadoStream` ganhou `stopReason?`.
- Commit `1837cf7` — **Lote 3 (PWA) CONCLUÍDO**: `src/app/manifest.ts`, `public/sw.js` (à mão, network-first + offline, nunca toca `/api/**`), `public/offline.html`, `RegistrarServiceWorker` (só produção), metadata iOS + `viewport` no layout, `scripts/gerar-icones-pwa.mjs` (usa `sharp`, já presente) + PNGs em `public/`.
- Commit `6f399cd` — **Lote 4 (mobile) CONCLUÍDO**: `max-lg:pl-16` no Header e no header do kanban; grids de localização/honorários responsivos; `ProximosPrazos` (fallback do calendário abaixo de xl); `KeyboardSensor` no dnd-kit.
- Commit `01da1b2` — **Lote 5 (curadoria) CONCLUÍDO**: 10 novos prompts RASCUNHO (réplica ×5, apelação ×4, recurso ordinário trabalhista ×1) via construtores em `_shared/`; snapshot de proteção travando os 20 prompts existentes + 20 novos; testes do registro atualizados. **Pendente:** revisão de curadoria jurídica humana dos 10 rascunhos antes de considerá-los prontos para produção. **Nota para B6:** a refatoração em 4 camadas NÃO foi feita (só os novos usam `_shared/construtores.ts`); os antigos seguem intactos e travados por snapshot.
