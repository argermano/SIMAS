import { REGRAS_FORMATACAO_FORENSE, SYSTEM_REGRAS_FORENSE } from '../regras-formatacao'

// Gerador GENÉRICO, ciente da ÁREA e do TIPO da peça. Usado como fallback quando
// não há prompt dedicado para (área, tipo). Substitui o fallback antigo, que usava
// sempre o prompt de "petição inicial previdenciária" — viés errado de área e de tipo.

function formatarQualificacao(q?: {
  autor?: Record<string, string | undefined>
  reu?: Record<string, string | undefined>
}): string {
  if (!q) return '### Qualificação das partes: Não disponível — use [PREENCHER] para dados faltantes.'

  const linhas: string[] = ['### Qualificação das partes (dados extraídos dos documentos):']

  if (q.autor && Object.values(q.autor).some(Boolean)) {
    linhas.push('**AUTOR/REQUERENTE:**')
    if (q.autor.nome) linhas.push(`- Nome: ${q.autor.nome}`)
    if (q.autor.cpf) linhas.push(`- CPF: ${q.autor.cpf}`)
    if (q.autor.rg) linhas.push(`- RG: ${q.autor.rg}${q.autor.orgao_expedidor ? ` (${q.autor.orgao_expedidor})` : ''}`)
    if (q.autor.estado_civil) linhas.push(`- Estado civil: ${q.autor.estado_civil}`)
    if (q.autor.nacionalidade) linhas.push(`- Nacionalidade: ${q.autor.nacionalidade}`)
    if (q.autor.profissao) linhas.push(`- Profissão: ${q.autor.profissao}`)
    const endereco = [q.autor.endereco, q.autor.bairro, q.autor.cidade, q.autor.estado, q.autor.cep].filter(Boolean).join(', ')
    if (endereco) linhas.push(`- Endereço: ${endereco}`)
    if (q.autor.email) linhas.push(`- E-mail: ${q.autor.email}`)
    if (q.autor.telefone) linhas.push(`- Telefone: ${q.autor.telefone}`)
  }

  if (q.reu && Object.values(q.reu).some(Boolean)) {
    linhas.push('**RÉU/REQUERIDO:**')
    if (q.reu.nome) linhas.push(`- Nome/Razão social: ${q.reu.nome}`)
    if (q.reu.cnpj_cpf) linhas.push(`- CNPJ/CPF: ${q.reu.cnpj_cpf}`)
    const endReu = [q.reu.endereco, q.reu.cidade, q.reu.estado].filter(Boolean).join(', ')
    if (endReu) linhas.push(`- Endereço: ${endReu}`)
  }

  linhas.push('')
  linhas.push('**OBRIGATÓRIO:** Use EXATAMENTE os dados acima na qualificação das partes. NÃO use [PREENCHER] para dados fornecidos aqui. Use [PREENCHER] APENAS para campos que NÃO aparecem na lista acima.')
  return linhas.join('\n')
}

export const SYSTEM_PECA_GENERICA = `Você é um advogado brasileiro sênior, generalista, redigindo peças processuais. Escreva a peça COMPLETA em Markdown bem formatado, com linguagem jurídica formal e técnica, fundamentada na legislação aplicável à área e ao tipo de peça. NUNCA interrompa a geração — sempre conclua a peça inteira. ${SYSTEM_REGRAS_FORENSE}`

export function buildPromptPecaGenerica(dados: {
  areaNome: string
  tipoNome: string
  analise?: Record<string, unknown>
  transcricao: string
  pedido_especifico?: string
  documentos: Array<{ tipo: string; texto_extraido: string; file_name: string }>
  localizacao?: { cidade?: string; estado?: string }
  qualificacao?: { autor?: Record<string, string | undefined>; reu?: Record<string, string | undefined> }
}): string {
  const comarca = dados.localizacao?.cidade && dados.localizacao?.estado
    ? `${dados.localizacao.cidade}/${dados.localizacao.estado}`
    : '[PREENCHER comarca]'

  return `
Você é um advogado brasileiro sênior. Redija a seguinte peça processual:
- ÁREA do Direito: ${dados.areaNome}
- TIPO de peça: ${dados.tipoNome}

## CONTEXTO
${dados.analise ? `### Análise jurídica prévia:\n${JSON.stringify(dados.analise, null, 2)}` : '### Sem análise prévia.'}

### Transcrição: ${dados.transcricao}
### Pedido específico: ${dados.pedido_especifico || 'Nenhum.'}
### Documentos: ${dados.documentos.length > 0 ? dados.documentos.map(d => `- ${d.file_name} (${d.tipo}): ${d.texto_extraido?.substring(0, 500) ?? 'sem texto'}`).join('\n') : 'Nenhum documento.'}
### Localização do cliente: ${dados.localizacao?.cidade ? comarca : 'Não informada'}
${formatarQualificacao(dados.qualificacao)}

## INSTRUÇÕES
- Redija a peça "${dados.tipoNome}" adequada à área de ${dados.areaNome}, com a ESTRUTURA, o ENDEREÇAMENTO e os REQUISITOS próprios desse tipo de peça e dessa área.
- Endereçamento ao juízo, tribunal ou órgão competente conforme a área/tipo (use [PREENCHER vara/órgão] e a Comarca de ${comarca} quando não houver dados).
- Use APENAS os fatos dos dados disponíveis; NÃO invente jurisprudência (marque com [VERIFICAR]); use [PREENCHER] para dados faltantes.
- Fundamente na legislação aplicável: CF/88, o código e as leis especiais da área (ex.: CDC, Lei do Inquilinato, Lei nº 8.112/90, CPP, CTN, Lei de Recuperação, conforme o caso).
- Linguagem técnica jurídica formal. GERE A PEÇA COMPLETA do início ao fim, sem interrupções.

${REGRAS_FORMATACAO_FORENSE}

Responda com a peça COMPLETA em Markdown bem formatado. Não interrompa a geração.
`.trim()
}
