...
export function BotaoExcluirPeca({ pecaId }: BotaoExcluirPecaProps) {
    ...
    return (
      <div className="flex items-center gap-1" onClick={e => e.preventDefault()}>
        <button
          onClick={excluir}
          disabled={excluindo}
          className="flex items-center gap-1 rounded-md bg-destructive px-2 py-1 text-xs font-medium text-white hover:bg-destructive/90 disabled:opacity-50 transition-colors"
        >
          {excluindo ? (
  ...
  return (
    <button
      ...
      className="shrink-0 rounded p-1 text-border hover:bg-destructive/5 hover:text-destructive transition-colors"
      title="Excluir peça"
    >
